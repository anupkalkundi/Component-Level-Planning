...
def normalize_formula_for_eval(formula, rules, variables=None):
    ...
    for key in known_keys:
        if "_width_" in key:
            formula = re.sub(rf"\b{re.escape(key.replace('_width_', '-width_'))}\b", key, formula, flags=re.IGNORECASE)
        if "_length_" in key:
            formula = re.sub(rf"\b{re.escape(key.replace('_length_', '-length_'))}\b", key, formula, flags=re.IGNORECASE)
...
def extract_formula_variables(formula, rules=None, variables=None):
    ...
    return sorted({normalize_variable(v) for v in found if v not in ignore_words})
...
def dynamic_resolve_and_evaluate(rule, all_rules, house_variables, previous_qty, product_qty_multiplier, rules_by_key, tracking_cache, active_path=None):
    ...
    comp_key = normalize_variable(slug(component))
    attr_key = normalize_variable(slug(attribute))
    lookup_identifier = f"{comp_key}_{attr_key}"

    if lookup_identifier in tracking_cache:
        return tracking_cache[lookup_identifier]

    if lookup_identifier in active_path:
        raise FormulaError(f"Circular dependency tracking path loop found: {lookup_identifier}")

    active_path.add(lookup_identifier)

    if rule_type in ["formula", "fomula"] and formula:
        dependencies = extract_formula_variables(formula, all_rules, house_variables)
        for dep in dependencies:
            if dep in rules_by_key and dep not in tracking_cache:
                dynamic_resolve_and_evaluate(
                    rules_by_key[dep], all_rules, house_variables, previous_qty,
    ...
    if rule_type in ["formula", "fomula"]:
        value = evaluate_formula(formula, house_variables, all_rules)
    elif needs_manual_dimension(rule, all_rules):
        value = get_dimension_value(component, attribute, house_variables, all_rules)
    else:
    ...
    value = apply_fixed_or_uploaded_component_value(component, attribute, value, all_rules)
    ...
    result_payload = {
        ...
        "normalized_formula": normalize_formula_for_eval(formula, all_rules, house_variables)
    }
    
    tracking_cache[lookup_identifier] = result_payload
    active_path.remove(lookup_identifier)
...
def show_component_calculator(conn, cur):
    ...
    unit_types = get_distinct_values(conn, cur, "unit_types", "unit_type", "WHERE project_name = %s", (project_name,))
    ...
    houses = get_distinct_values(conn, cur, "houses", "house_number", "WHERE project_name = %s AND unit_type = %s", (project_name, unit_type))

    with col3:
        selected_houses = st.multiselect("House Number", houses, default=houses[:1])
    ...
    if uses_orientation:
        ...
        st.info(f"Total LH Quantity: {total_lh_quantity} | Total RH Quantity: {total_rh_quantity} | Total Quantity: {total_product_quantity}")
    else:
    ...
    if input_keys:
        ...
        for idx, key in enumerate(input_keys):
            with input_cols[idx % 4]:
                if key == "mesh_yes":
                    mesh_choice = st.selectbox("Mesh", ["No", "Yes"], key=f"input_{state_key}_{key}")
    ...
    if st.button("Generate Components", key=f"generate_components_{state_key}"):
        ...
        rules_by_key = {}
        for rule in rules:
            c_name = normalize_variable(slug(rule_value(rule, "component")))
            a_name = normalize_variable(slug(rule_value(rule, "attribute")))
            rules_by_key[f"{c_name}_{a_name}"] = rule
        ...
        for house_number in selected_houses:
            ...
            for rule in rules:
                ...
                try:
                    calc_res = dynamic_resolve_and_evaluate(
                        rule, rules, house_variables, previous_qty,
                        product_qty_multiplier, rules_by_key, tracking_cache
                    )

                    value = calc_res["value"]
                    component_qty = calc_res["component_qty"]
                    total_quantity = calc_res["total_quantity"]
                    normalized_formula = calc_res["normalized_formula"]
