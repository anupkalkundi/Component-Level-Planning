...
def calculate_cft(length, width, thickness, qty):
    length = clean_number(length) or 0
    width = clean_number(width) or 0
    thickness = clean_number(thickness) or 0
    qty = clean_number(qty) or 0

    if length <= 0 or width <= 0 or thickness <= 0 or qty <= 0:
        return Decimal("0")

    return Decimal(str(round((length * width * thickness * qty) / 1728, 2)))
...
def build_component_summary(rules, calculated_rows, product_qty):
    ...
    for component, component_rules in rules_by_component.items():
        ...
        cft = calculate_cft(length, width, thickness, total_qty)
...
def show_component_calculator(conn, cur):
        ...
        for idx, key in enumerate(formula_inputs):
            with formula_cols[idx % 4]:
                user_inputs[key] = st.number_input(
                    ...
                    step=1.0,
                    format="%.2f",
                    key=f"formula_input_{product_code}_{key}"
                )
        ...
        for idx, item in enumerate(manual_inputs):
            with manual_cols[idx % 4]:
                user_inputs[item["key"]] = st.number_input(
                    ...
                    step=1.0,
                    format="%.2f",
                    key=f"manual_input_{product_code}_{item['key']}"
                )
    ...
    if selected_houses:
        ...
        for idx, house_number in enumerate(selected_houses):
            with qty_cols[idx % 4]:
                house_quantities[house_number] = st.number_input(
                    f"{house_number} Quantity",
                    min_value=1,
                    value=1,
                    step=1,
                    key=f"product_qty_{product_code}_{house_number}"
                )
    ...
    if st.button("Generate Components", type="primary"):
        ...
        try:
            calculated_rows, variables = calculate_rules(rules, user_inputs)

            for house_number in selected_houses:
                product_qty = int(house_quantities.get(house_number, 1))
    ...
    if "component_preview_rows" in st.session_state:
        ...
        st.info(f"Total CFT: {round(total_cft, 2)}")
